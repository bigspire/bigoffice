$(document).ready(function() { 	
	...
	*/
	...
	// show half day div in page load
	 ...
	 */
	 function validateForm (form, action) { 
		...
		$(".form-vertical input[type=text], select, input[type=checkbox]").each(function(index,elem){			
			...
			// for not draft 
			if(action != 'draft'){
				 // validate amount field for > 50 rupees
				if($(this).val() > 50 &&  $(this).hasClass('amtExp') ) {
					var get_id = $(this).attr('id');
					bill_id =  get_id.split('_');
					bill_box = '#billref_'+bill_id[1];
					if(bill_id[0] == 'amt' && $.trim($(bill_box).val()) == ''){
						$(bill_box).addClass('required missing');
						submit = false;
					}				
				}else if($(this).val() < 50 &&  $(this).hasClass('amtExp') ) {
					var get_id = $(this).attr('id');
					bill_id =  get_id.split('_');
					if(bill_id[0] == 'amt'){
						bill_box = '#billref_'+bill_id[1];
						$(bill_box).removeClass('required missing');
					}
				}
			}
