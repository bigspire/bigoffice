			...
			if(!mysql_num_rows($result)){
				...
				if(mysql_num_rows($query)){
					...
					while($row = mysql_fetch_assoc($query)){
						...
						if($explode[2] == 'mypdca.in'){$val = $row['value']; $option_ext = "<img style='border:1px dotted #efefef;vertical-align:middle' src='$val'>";}else{$option_ext = ucwords($row['value']);}
						echo '<p style="margin-bottom:3px;"><input type="radio"  val="'.$row['value'].'" name="poll" class="otherCls" value="'.$row['id'].'" id="option-'.$row['id'].'" /> 
						<label for="option-'.$row['id'].'" >'.$option_ext.'</label></p>';						
					}
				...
				if(mysql_num_rows($query)){
					while($row = mysql_fetch_assoc($query)){
						$explode = explode('/',$row['value']);
						if($explode[2] == 'mypdca.in'){$val = $row['value']; $option_ext = "<img src='$val'>";}else{$option_ext = ucwords($row['value']);}
						//$cls = $row['value'] == 'Others' ? 'otherCls' : '';
...
function showresults($poll_id){
	...
	while($row=mysql_fetch_assoc($query)){
		...
		$explode = explode('/',$row['value']);
		if($explode[2] == 'mypdca.in'){$val = $row['value']; $option_ext = "<img style='border:1px dotted #efefef;vertical-align:middle' src='$val'>";}else{$option_ext = ucwords($row['value']);}
		echo '<div class="option" ><p style="margin-bottom:3px">'.$option_ext.' (<em>'.$percent.'%, '.$row['poll_votes'].' votes</em>)</p>';
		echo '<div class="bar ';
