...
try{
	...
	if($count == 0){
		$alert_msg = 'This login type is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Title','Description','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E'), $field = array('title','description','created_date','modified_date','status'),'Login Types');
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = 'Login type ' . $_GET['status'] . ' successfully';
	}
