...
try{
	...
	if($count == 0){
		$alert_msg = 'This ticket details is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Employee Name','Subject','Type','Priority','Description','Attached File','Created Date','Status') ,$col = array('A','B','C','D','E','F','G','H'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G','H'), $field = array('emp_name','subject','type','priority','description','attach_file','created_date','status'),'Tickets');
	}  
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = 'Ticket ' . $_GET['status'] . ' successfully';
	}
