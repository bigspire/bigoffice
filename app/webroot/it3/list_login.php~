...
try{
   ...
   $count = $data_num['total'];
	if($count == 0){
		$alert_msg = 'This login details is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Employee Name','Login Type','User Name','Password','Server','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E','F','G','H'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G','H'), $field = array('full_name','title','user_name','password','host','created_date','modified_date','status'),'Logins');
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = 'Login details ' . $_GET['status'] . ' successfully';
	}	
