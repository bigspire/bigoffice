...
try{
	...
	if($count == 0){
		$alert_msg = 'This software details is not in our database';
	}
...
$sort_fields = array('1' => 'software_type','brand','edition','no_license','subscription','validity','created','modified');
$org_fields = array('1' => 'st.title', 'b.title', 's.edition', 's.no_license','s.subscription',
's.validity_year','s.created_date','modified_date');
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){
      ...
      $excelObj->printHeader($header = array('Software Type','Brand','Edition','No Of License','Subscription','Validity Year','Validity Till','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E','F','G','H','I','J'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G','H','I','J'), $field = array('software_type','brand','edition','no_license','subscription','validity_year','validity_date','created_date','modified_date','status'),'Software');
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated' ){
  		$alert_msg = 'Software details ' . $_GET['status'] . ' successfully';
	}
