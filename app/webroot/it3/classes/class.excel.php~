...
class libExcel{
	...
	function printHeader($header,$col){    
        ...
        $this->objPHPExcel->getActiveSheet()->setCellValue($col[$j] . $i,  $header[$j])->getStyle($col[$j] . $i,  $header[$j])->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E05CC2')))); 
        $this->objPHPExcel->getActiveSheet()->getStyle('1:1')->getFont()->setBold(true);
      } 
    	$i++; 
	}	
