...
$query = "CALL it_list_hardware('".$keyword."','".$hw_type."','".$hw_status."','".$from_date."','".$to_date."','0','0','','','".$_GET['action']."')";
try{
	...
	if($count == 0){
		$alert_msg = 'This hardware details is not in our database';
	}
...
$sort_fields = array('1' => 'hardware_type','brand','model_id','inventory_no','location','asset_desc','modified','created');
$org_fields = array('1' => 'ht.title', 'b.title', 'h.model_id', 'h.inventory_no','h.vendor_city','h.asset_desc','modified_date','created_date');
...
// fetch all records
$query = "CALL it_list_hardware('".$keyword."','".$hw_type."','".$hw_status."','".$from_date."','".$to_date."','$start','$limit',
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Type','Brand','Model Id','Inventory No','Location','Asset Description','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E','F','G','H','I'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G','H','I'), $field = array('type','brand','model_id','inventory_no','location','asset_desc','created_date','modified_date','status'),'Hardwares');
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated' || $_GET['status'] == 'moved'){
  		$alert_msg = 'Hardware details ' . $_GET['status'] . ' successfully';
	}
