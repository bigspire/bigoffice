   ...
   Date : 20-06-2016 *}
	...
	<div class="container-fluid" id="content">
		<div id="main">
           ...
           <a href="list_change_asset_info.php"><button style="margin-bottom:9px;margin-left:4px;" type="button" class="btn btn-primary"><i class="icon-refresh"></i> Reset</button></a>
           <a href="list_change_asset_info.php?action=export&keyword={$smarty.post.keyword}&emp_name={$smarty.post.emp_name}&asset_type={$smarty.post.asset_type}&f_date={$smarty.post.f_date}&t_date={$smarty.post.t_date}"><button type="button" class="btn btn-primary" style="float:right;margin-right:20px;"><i class="icon-reply"></i> Export</button></a>							
           </div>			
					<table class="table table-hover table-nomargin table-bordered usertable dataTable">
									<thead>	
										<tr>
											<th width="50">
											<a href="list_change_asset_info.php?field=emp_name&order={$order}&page={$smarty.get.page}&keyword={$keyword}&emp_name={$emp_name}&asset_type={$asset_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_emp_name}">Employee Name</a></th>
										   <th width="50">
											<a href="list_change_asset_info.php?fieldasset_type&order={$order}&page={$smarty.get.page}&keyword={$keyword}&emp_name={$emp_name}&asset_type={$asset_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_asset_type}">Asset Type</a></th>		
											<th width="50">
											<a href="list_change_asset_info.php?field=message&order={$order}&page={$smarty.get.page}&keyword={$keyword}&emp_name={$emp_name}&asset_type={$asset_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_message}">Message</a></th>												
											<th width="50">
											<a href="list_change_asset_info.php?field=created_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&emp_name={$emp_name}&asset_type={$asset_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_created_date}">Created Date</a></th>												
											<th width="50">
											<a href="list_change_asset_info.php?field=modified&order={$order}&page={$smarty.get.page}&keyword={$keyword}&emp_name={$emp_name}&asset_type={$asset_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_modified}">Modified Date</a></th>													
