...
try{
	...
	if($count == 0){
		$alert_msg = 'This brand is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Title','Type','Created Date','Status') ,$col = array('A','B','C','D'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('title','type','created_date','status'),'Brands'); 				
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $alert_msg = 'Brand details ' . $_GET['status'] . ' successfully';
	}
