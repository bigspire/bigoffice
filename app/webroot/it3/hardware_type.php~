...
try{
	...
	if($count == 0){
		$alert_msg = 'This hardware type is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Title','CT Code','Description','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E','F'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F'), $field = array('title','ctcode','description','created_date','modified_date','status'),'Hardware Types');
		die;
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = 'Hardware type ' . $_GET['status'] . ' successfully';
	}
