...
try{
	...
	if($count == 0){
		$alert_msg = 'This software type is not in our database';
	}
...
try{
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Title','Description','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E'), $field = array('type','description','created_date','modified_date','status'),'Software Types');
	}
	...
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = 'Software type ' . $_GET['status'] . ' successfully';
	}
