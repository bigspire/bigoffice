...
$query = "CALL it_list_change_asset('".$keyword."','".$emp_name."','".$asset_type."','".$from_date."','".$to_date."','0','0','','','".$_GET['action']."')";
...
// fetch all records
$query = "CALL it_list_change_asset('".$keyword."','".$emp_name."','".$asset_type."','".$from_date."','".$to_date."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list page');
	}
	...
	if(($_GET['action'] == 'export') && ($count > 0)){
      ...
      $excelObj->printHeader($header = array('Employee Name','Type','Message','Created Date','Modified Date','Status') ,$col = array('A','B','C','D','E','F'));  
		...
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F'), $field = array('full_name','type','message','created_date','modified_date','status'),'Change Asset Info');
		die;
	}
	...
	if($_GET['status'] == 'updated'){
  		$alert_msg = 'Change asset info ' . $_GET['status'] . ' successfully';
	}
