...
$from_date = $fun->convert_date($_POST['f_date']);
$to_date = $fun->convert_date($_POST['t_date']);
...
$query = "CALL it_list_assign_asset('".$keyword."','".$emp_name."','".$asset_type."','".$from_date."','".$to_date."','0','0','','','".$_GET['action']."')";
...
$sort_fields = array('1' => 'emp_name','asset_type','field','field','brand','created_date');
$org_fields = array('1' => 'emp_name','asset_type','edition','hw_type','brand','it_asset.created_date');
...
try{
	...
	while($obj = $mysql->display_result($result)){
 		$emp_name_drop[] = $obj['first_name'].' '.$obj['last_name'];
	}
...
// fetch all records
$query = "CALL it_list_assign_asset('".$keyword."','".$emp_name."','".$asset_type."','".$from_date."','".$to_date."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
	...
	while($obj = $mysql->display_result($result))
	{
		...
		$data[$i]['asset_type'] = $fun->it_brand_type($obj['asset_type']);
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Employee Name','Asset Type','Edition','Software Type','Hardware Type','Brand','Created Date') ,$col = array('A','B','C','D','E','F','G'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G'), $field = array('emp_name','asset_type','edition','sw_type','hw_type','brand','created_date'),'Assigned Assets');
		die;
	}	
	...
	if($_GET['status'] == 'created' || $_GET['status'] == 'updated'){
  		$alert_msg = $fun->it_brand_type($_GET['asset_type']).' '.   $_GET['status'] . ' for '  .  $_GET['name']  .  ' successfully';
	}
	if($_GET['status'] == 'deleted'){
		$alert_msg = $fun->it_brand_type($_GET['asset_type']).' '.   $_GET['status'] .  ' successfully';
	}
