...
class TskprojectrequestController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Project Requests - Work Planner - BigOffice');
		// load team members
		$this->load_employee();
	...
	public function process_req($req_id, $status,$user_id){ 
		...
		if($ret_value == 'pass'){	
			...
			if($this->TskProjectRequest->save($data, true, $fieldList = array('approved_by','approved_date','remarks','status'))){				
				...
				// notify employee						
				if(!$this->send_email('BigOffice - Your project request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_project_req', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){		
					// show the msg.								
	...
	public function view_project_req($id){
		// set the page title		
		$this->set('title_for_layout', 'Approve Project Request - Work Planner - BigOffice');
