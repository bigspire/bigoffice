...
class HrAttendanceController  extends AppController {  
	...
	public function index(){
		// set the page title
		$this->set('title_for_layout', 'Attendance - HRIS - BigOffice');
		...
		$this->get_used_leaves($emp_data['HrEmployee']['id'],$emp_data['HrEmployee']['doj'],$emp_data['HrEmployee']['emp_type'],$emp_data['HrEmployee']['doc'],$emp_data['HrEmployee']['gender']);
		...
		$this->check_sat_working($emp_data['HrEmployee']['id']);
		...
		$this->calculate_late_deduct($emp_data['HrEmployee']['id'],$this->request->data['HrAttendance']['att_year']['year']);
		// get attendance late timing for add
		$this->calculate_late_addition($emp_data['HrEmployee']['id'],$this->request->data['HrAttendance']['att_year']['year']);
				...
				if($this->HrWaiveOff->save($this->request->data['HrWaiveOff'])){
					...
					foreach($superior_data as $superior){
						if($superior['HrEmployee']['id'] != $this->Session->read('USER.Login.id')){
							...
							$sub = 'BigOffice - '.$emp_data['HrEmployee']['first_name'].' '.$emp_data['HrEmployee']['last_name'].' Late Minutes Waived!';
