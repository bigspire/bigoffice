...
class HrEmployeeController extends AppController {  
	...
	public function index(){
		// set the page title
		$this->set('title_for_layout', 'Employee - HRIS - BigOffice');
		...
		// fetch the advances		
		$this->paginate = array('fields' => array('id','first_name', 'emp_no','last_name','email_address','HrDepartment.dept_name','HrDesignation.desig_name','photo_status', 'photo','status','created_date','doj','probation','doc','emp_type'),'limit' => 50,'conditions' => array($keyCond,$stCond,$recCond,$typeCond, 'HrEmployee.is_deleted' => 'N'), 'order' => array('first_name' => 'asc'), 'group' => array('HrEmployee.id'));
		...
		// update pl details
		 ...
		 if($this->Session->read('USER.Login.id') == 'RAVI'){
			$this->loadModel('HrEmployee');
			$data = $this->HrEmployee->find('all', array('fields' => array('id', 'HrEmployee.first_name',
			'HrEmployee.last_name','hr_branch_id'), 'conditions' => array('HrEmployee.emp_type' => 'A',
			'HrEmployee.status' => '1',
			'HrEmployee.is_deleted' => 'N', 
			'HrEmployee.doj'), 'group' => array('HrEmployee.id'), 'order' => array('HrEmployee.first_name' => 'asc')));
			echo '<table>';
			foreach($data as $rec){
				$count = $this->eval_leave_status($rec['HrEmployee']['id'],$rec['HrEmployee']['hr_branch_id'],'2016-12',$rec['HrEmployee']['doj']);
				echo '<tr>';
				$pal_bal = round($count / 20);
				/* 
				$data2 = array('app_users_id' => $rec['HrEmployee']['id'], 'pl_bal' => $pal_bal, 'nbl_bal' => '0',  'created_date' => $this->Functions->get_current_date(), 'created_by' => '54',
				 'year' => date('Y'));
				...
				*/	
				...
				// echo '<td>'.$rec['HrEmployee']['first_name'].' '.$rec['HrEmployee']['last_name'].'</td>';
				// echo '<td>'.$count.'</td>';
				echo '<td>'.$pal_bal.'</td>';
				echo '<td>0</td>';
				// echo '<td>NULL</td>';
				echo '<td>2017-01-01</td>';
				//echo '<td>54</td>';
				//echo '<td>NULL</td>';
				//echo '<td>NULL</td>';
				//echo '<td>N</td>';
				echo '<td>'.$rec['HrEmployee']['id'].'</td>';
				echo '</tr>';
			}
			echo "</table>";
			die;
	...
	public function create_employee($type){ 
		// set the page title		
		$this->set('title_for_layout', 'Create Employee - HRIS - BigOffice');	
		// load static models
		...
		$this->set('emp_types', array('R' => 'Regular','A' => 'Associate 1', 'A2' => 'Associate 2'));
		$this->set('att_types', array('B' => 'Biometric','O' => 'Online','N' => 'Not Applicable'));
	...
	public function edit_employee($type, $id){	
		// set the page title		
		$this->set('title_for_layout', 'Edit Employee - HRIS - BigOffice');	
		// load static models
		$this->load_static_tables();
		$this->set_year_passing();	
		$this->set_experience();
	...
	public function view_employee($id, $type){
		// set the page title		
		$this->set('title_for_layout', 'View Employee - HRIS - BigOffice');
	...
	public function verify_photo($id, $status){
		...
		// send mail		
		if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.	ucfirst($this->Session->read('USER.Login.last_name')).  ' '.$st_msg.' profile photo!', 'photo_verification', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){	
		// show the msg.								
