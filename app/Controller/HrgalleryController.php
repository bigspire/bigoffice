...
class HrgalleryController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Gallery - HRIS - BigOffice');		
	...
	public function create_gallery(){ 
		// set the page title		
		$this->set('title_for_layout', 'Create Gallery - HRIS - BigOffice');	
		$time = time();	
					...
					if(!empty($hr_data)){
						...
						if($this->HrGalStatus->save($data, true, $fieldList = array('hr_gallery_id','created_date','app_users_id'))){							
							...
							$vars = array('from_name' => ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')),'name' => $hr_data['Home']['first_name'].' '.$hr_data['Home']['last_name'], 'desc' => $this->request->data['HrGallery']['desc'], 'title' => $this->request->data['HrGallery']['title']);
								// notify superiors						
								if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' submitted gallery!', 'gallery_creation', 'noreply@mypdca.in', $hr_data['Home']['email_address'],$vars)){	
								// show the msg.								
	...
	public function view_gallery($id){
		// set the page title		
		$this->set('title_for_layout', 'View Gallery - HRIS - BigOffice');
