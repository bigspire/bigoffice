...
class TvlbooktktController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Book Ticket - Biz Tour - BigOffice');		
	...
	public function export(){
		$this->set('title_for_layout', 'Export Travel - Biz Tour - BigOffice');		
		...
		if($this->request->is('post')){ 
			...
			if ($this->TvlBookTkt->validates(array('fieldList' => array('avail','tvl_mode_id','book_mode','issue_date','suggestion')))) {
				...
				if($this->TvlBookTkt->TvlTicketStatus->save($this->request->data['TvlBookTkt'])){	
					...
					// notify employee						
					if(!$this->send_email('BigOffice - Ticket Status Updated - '.$tvl_data['TvlBookTkt']['tvl_code'], 'ticket_status', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){		
						// show the msg.								
	...
	public function notify_tvl_desk($approver){
		...
		foreach($approval_data as $tvl_data){
			if($superior_data['HrEmployee']['id'] != $tvl_data['HrEmployee']['id']){
				...
				// notify superiors						
				if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'))." created travel request $sub_action!", 'travel_creation', 'noreply@mypdca.in', $tvl_data['HrEmployee']['email_address'],$vars)){	
					// show the msg.								
		...
		if($this->request->is('post')){
			...
			if ($this->TvlBookTkt->TvlTicket->validates(array('fieldList' => array('amount','tkt1', 'book_via','book_ref_no','agent_copy1','book_date')))) {
				...
				if($this->TvlBookTkt->TvlTicket->save($this->request->data, array('validate' => false))){
					...
					// notify employee						
					if(!$this->send_email('BigOffice - Ticket Confirmation - '.$tvl_data['TvlBookTkt']['tvl_code'], 'book_ticket', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars,$attachment)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
						}
