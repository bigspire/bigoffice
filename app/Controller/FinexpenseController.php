...
class FinExpenseController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', $this->exp_type.' - Finance - BigOffice');
	...
	public function create_expense(){ 
		// set the page title		
		$this->set('title_for_layout', 'Create Expense - Finance - BigOffice');	
			...
			if($this->request->data['FinExpense']['is_draft'] == 'N'){
				...
				if(!empty($approval_data)){
					...
					foreach($approval_data as $fin_data){ 
						...
						// notify superiors						
						if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' submitted expense request!', 'expense_creation', 'noreply@mypdca.in', $fin_data['Home']['email_address'],$vars)){	
							// show the msg.								
	...
	public function edit_expense($id){ 
		// set the page title		
		$this->set('title_for_layout', 'Edit '.$this->exp_type.'  - Finance - BigOffice');
	...
	public function view_expense($id){
		// set the page title		
		$this->set('title_for_layout', 'View Expense - Finance - BigOffice');
		if(!empty($id) && intval($id)){
			// authorize user before action
			$ret_value = $this->auth_action($id);
			if($ret_value == 'pass'){	
