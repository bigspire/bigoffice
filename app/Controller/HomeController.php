...
class HomeController extends AppController {  
	...
	public function index(){	
		...
		// set the page title
		$this->set('title_for_layout', 'Home - BigOffice');				
		...
		*/
		...
		$data = $this->HrAttendance->HrEmployee->find('all', array('fields' => array('HrEmployee.full_name','HrEmployee.emp_no'),
		'group' => array('HrEmployee.full_name','emp_no'), 'conditions' => 	array("OR" => array 
		('emp_no like' => '%'.$q.'%','HrEmployee.full_name like' => '%'.$q.'%'), 
		'AND' => array('HrEmployee.is_deleted' => 'N'))));		
	...
	public function notify_hr($reason){
		...
		foreach($hr_data as $hr){	
			...
			if($this->Session->read('USER.Login.id') != $hr['Home']['id']){
				$sub = 'BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' submitted late attendance request!';
		...
		if($this->request->is('post')){
			...
			if($this->Functions->validate_img($_FILES['file']['type'], $_FILES['file']['size'])){
				...
				if($this->Functions->upload_file($src, $dest)){
					...
					// notify hr
					$sub = 'BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.	ucfirst($this->Session->read('USER.Login.last_name')).' updated profile photo!';
	...
	public function feedback(){ 
		...
		// notify superiors						
		if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' submitted feedback!', 'feedback_request', $this->Session->read('USER.Login.email_address'), 'helpdesk@career-tree.in',$vars)){		
			// show the msg.								
	...
	public function report_issue(){ 
		...
		// notify superiors						
		if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' submitted an issue!', 'issue_request', $this->Session->read('USER.Login.email_address'), 'helpdesk@career-tree.in',$vars)){		
			// show the msg.								
			$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending mail...', 'default', array('class' => 'alert alert-error'));				
		}else{	
	...
	public function change_req(){ 
		...
		if (!empty($this->request->data)){
			...
			if($this->HrChgReq->save($data, true, $fieldList = array('type','created_date','app_users_id','desc'))){
				// notify hr
				$sub = 'BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.	ucfirst($this->Session->read('USER.Login.last_name')).' submitted change request!';
	...
	public function company($page){
		// set the page title
		$this->set('title_for_layout', $page.' - BigOffice');	
	...
	public function show_by(){
		...
		*/
		...
		// check saturday working
		$this->check_sat_working($this->Session->read('USER.Login.id'));
		...
		// get waive off req.
		$this->loadModel('HrReqWaive');
		$data = $this->HrReqWaive->find('count', array( 'conditions' => array('is_draft' => 'N',
		"date_format(date, '%Y-%m')"  => date('Y-m'),
		'app_users_id' => $this->Session->read('USER.Login.id'))));
		$this->set('waive_done', $data);
	...
	/* function to create waive off request */
	public function view_waiveoff(){
		$this->layout = 'iframe_dash'; 
		// get the survey details
		$this->loadModel('HrReqWaive');		
		$data = $this->HrReqWaive->find('all', array('fields' => array('HrReqWaive.id','date','reason','status'), 'conditions' => 
		array('is_draft' => 'N', 'app_users_id' => $this->Session->read('USER.Login.id'), "date_format(date, '%Y-%m')"  => date('Y-m')),
		'order' => array('HrReqWaive.date' => 'asc'), 'group' => array('HrReqWaive.id')));
		$this->set('waive_data', $data);
	}
	
	
	/* function to create waive off request */
	public function request_waiveoff(){
		$this->layout = 'iframe_dash'; 
		// get the survey details
		$this->loadModel('HrReqWaive');		
		$data = $this->HrReqWaive->find('all', array('fields' => array('HrReqWaive.id','date','reason'), 'conditions' => 
		array('is_draft' => 'Y', 'app_users_id' => $this->Session->read('USER.Login.id')),'order' => array('HrReqWaive.date' => 'asc'), 'group' => array('HrReqWaive.id')));
		$this->set('waive_data', $data);		
		if (!empty($this->request->data)){ 	
			$date_split = explode('/',$this->request->data['HrReqWaive']['date0']);
			$date = $date_split[2].'-'.$date_split[1].'-';
			// check if draft
			if($this->request->data['HrReqWaive']['is_draft'] == '1'){			
				// remove waive request
				$this->remove_waive_msg($date);
				$this->save_waive_msg('Y');
				$this->set('waive_submit', 1);
				$this->get_waive_msg($date);
			}else{
				$this->HrReqWaive->set($this->request->data);
				// remove waive request
				$this->remove_waive_msg($date);
				// save the waive request
				$save = $this->save_waive_msg('N');		
				$this->set('waive_complete', 1);
				$this->set('waive_close', 3);
			}			
		}
		
	}
	
	/* function to remove the waive msg */
	public function remove_waive_msg($date){
		$this->HrReqWaive->deleteAll(array('date like' => $date.'%', 'app_users_id' => $this->Session->read('USER.Login.id')), false);
	}
	
	
	/* function to save waive msg */
	public function save_waive_msg($draft){
		$tot = $this->request->data['HrReqWaive']['tot_msg'];
		$save  = true;
		for($i = 0; $i <= $tot; $i++){
			$format_date = $this->Functions->format_date_save($this->request->data['HrReqWaive']['date'.$i]);
			// function to check the waive off date exists
			$check = $this->check_waive_exist($format_date);
			// $check_att = $this->check_attendance_marked($format_date);
			if(!$check){
				if(!empty($this->request->data['HrReqWaive']['reason'.$i]) && !empty($this->request->data['HrReqWaive']['date'.$i])){
					$this->request->data['HrReqWaive']['reason'] = $this->request->data['HrReqWaive']['reason'.$i];
					$this->request->data['HrReqWaive']['date'] = $format_date;
					$this->request->data['HrReqWaive']['created_date'] = $this->Functions->get_current_date();
					$this->request->data['HrReqWaive']['app_users_id'] = $this->Session->read('USER.Login.id');
					$this->request->data['HrReqWaive']['is_draft'] = $draft;
					$this->HrReqWaive->create();
					if(!$this->HrReqWaive->save($this->request->data['HrReqWaive'])) {	
						$save = false;
					}
				}
			}
	...
	/* function to check the waive exist */
	public function check_waive_exist($date){
		$total = $this->HrReqWaive->find('count', array('conditions' => array('date' => $date, 
		'app_users_id' => $this->Session->read('USER.Login.id'))));
		return $total;
	}
	...
	/* function to get the waive off request */
	public function get_waive_msg($date){
		$data = $this->HrReqWaive->find('all', array('fields' => array('date','reason'), 'conditions' => array('date like' => $date.'%', 
		'app_users_id' => $this->Session->read('USER.Login.id')), 'order' => array('HrReqWaive.id' => 'asc')));
		$this->set('waive_data', $data);
	}
	...
	/* function to display the HR Processes */
	public function hr_spoc(){
		$this->layout = 'iframe';
	}
	
	/* function to display the HR Processes */
	public function fin_guideline(){
		$this->layout = 'iframe';
	}
