...
class FinAdvPayController extends AppController {  
	...
	public function index(){
		...
		// set the page title
		$this->set('title_for_layout', 'Pay Advance - Finance - BigOffice');
		// get employee list
		$this->loadModel('Home');
	...
	public function notify_employee($user_data,$pay_data,$adv_data){	
		...
		if(!$this->send_email('BigOffice - Advance payment updated by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_adv_pay', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
			// show the msg.								
