...
class HrprofilechangeController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Profile Change - HRIS - BigOffice');
		...
		if (!empty($this->request->data)){ 
			...
			if ($this->HrProfileChange->validates(array('fieldList' => array('status')))) {
					...
					if($this->HrProfileChange->save($this->request->data['HrProfileChange'])) {	
						// notify user in mail
						$sub = 'BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.	ucfirst($this->Session->read('USER.Login.last_name')).' updated your profile!';
