...
class HrplreqController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve PL Requests - HRIS - BigOffice');
		// load team members
		$this->load_employee();
	...
	public function process_req($req_id, $status,$user_id){ 
		...
		if($ret_value == 'pass'){
			...
			if($this->HrPlReq->save($data, true, $fieldList = array('approve_by','approve_date','remarks','status'))){				
				...
				// notify employee						
				if(!$this->send_email('BigOffice - Your PL request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_pl_req', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){		
					// show the msg.								
	...
	public function view_request($id){
		// set the page title		
		$this->set('title_for_layout', 'View PL Request - HRIS - BigOffice');
