...
class AppController extends Controller {
	...
	public function beforeRender(){ 
		...
		if($this->request->params['controller'] != 'Logins' && $this->request->params['controller'] != 'hrpayslip' && $this->request->params['controller'] != 'hrreminder'){ 
			if($this->check_session()){
				...
				$this->set('WAIVE_REQ_COUNT', $this->waive_count);
	...
	public function check_mobile_device(){
		...
		if($this->Session->read('USER.Login.id') != '54' && $this->Session->read('USER.Login.app_roles_id') != '18' && $this->Session->read('USER.Login.id') != '72'){
			if($this->request->is('mobile')){
				$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert-error">&times;</button>Oops! BigOffice is not available in mobile!', 'default', array('class' => 'alert alert-error'));
				$this->invalid_access();
			}
		}
	}
		...
		foreach($modules as $key => $module){ 
			...
			if (in_array($module['Module']['id'], $format_per)) { 	
				switch($module['Module']['id']){
					...
					break;
					case 103:					
					$this->set('upload_att_menu', 1);
					break;
					case 104:					
					$this->set('deactivate_sat', 1);
					break;
					case 105:					
					$this->set('approve_waiveoff_change_menu', 1);
					$this->waive_req_count();
					break;
	...
	/* function to show waive req. count */
	public function waive_req_count(){
		// check finance modules
		if($this->check_hr_module()){
			// fetch the recent users approval req.	
			$this->loadModel('HrAttWaive');
			$this->waive_count = $this->HrAttWaive->find('count', array('conditions' => array('HrAttWaive.is_draft' => 'N','HrAttWaive.status' => 'P'),
			'group' => array('HrEmployee.id',"date_format(HrAttWaive.date, '%Y-%m')")));
			$this->emp_count += $this->waive_count;
			$this->hr_count += $this->waive_count;
			$this->set('WAIVE_REQ_COUNT', $this->waive_count);
		}
	}
	...
	public function check_hr_module(){
		$controller =  $this->params['controller'];
		$modules = array('home', 'hrhome', 'hrleave', 'hrpermission', 'hrcancelleave', 'hraprcancelleave',
		'hrleaveapprove','hrperapprove', 'roles', 'hremployee', 'approve', 'hrcompany','hrdepartment',
		'hrdesignation','hrattendance','hrform','hrlatest', 'bank','hrorg','hrbank','hrbankacc',
		'hruploadpay','hrmypayslip','hrholiday','hrgallery','hrgalapprove','hrattchange','hraprattchange',
		'hrprofilechange','hrpoll','hrattchangeapprove','hrbranch','hrbusinessunit','hrofficetiming',
		'hrleavedetails','hrreport','hrplreq','hrmessage','hrsurvey','hrvoice','hruploadatt','hrdeactivatesat',
		'hrattwaive');
		if(in_array($controller, $modules)){ 
			return true;
		}else{	
	...
	/* function to disable attendance */
	public function check_att_available(){
		if($this->Session->read('USER.Login.att_type') == 'N'){
			return false;
		}else{
			return true;
		}
	...
	public function update_att_change($from){
		...
		if($this->check_hr_module() && $this->check_att_available()){
	...
	public function get_used_leaves($id, $doj,$emp_type,$doc,$gender){ 
		$id = $this->get_user_id($id);
		$this->get_used_lve($id,$emp_type,$gender);
		//if(date('Y') == '2014'){
	...
	public function load_leave_types($gender, $id, $doj,$emp_type){
		...
		*/
		...
		// print_r($leave_detail);
		$this->set('leaveDetail', $leave_detail);
		
		// for drop down
		$leave_list = $this->HrLeave->HrLeaveType->find('list', array('fields' => array('id','leave_type'), 'order' => array('priority ASC'),'conditions' => array('status' => 1, 'is_deleted' => 'N', $cond, $leave_cond)));
	...
	public function get_spl_avail($id){ 
		...
		$no_spl_leaves = $data[0][0]['no_days'];
		...
		$remain_spl = $remain_spl > 10 ? 10 : $remain_spl;
	...
	/* function to get nbl */
	public function get_used_lve($id, $emp_type,$gender){
		$this->HrLeave->unBindModel(array('hasOne' => array('HrLeaveStatus')));
		// for associate conditions
		if($emp_type == 'A' || $emp_type == 'A2' || $this->probation == 'Y'){
			$date_cond = date('Y-m');
		}else{
			$date_cond = date('Y');
		}
	...
	/* function to check saturday working */
	public function check_sat_working($id, $date){
		$date = $date == '' ? date('Y-m-d') : $date;
		$this->loadModel('HrDeactivateSat');
		$date_cond = array('and' => array('from_date <=' => $date),  array('to_date >=' => $date));
		$check_sat = $this->HrDeactivateSat->find('count', array('conditions' => array($date_cond,
				'app_users_id' => $id)));
		$this->set('satWork', $check_sat);
		return $check_sat;
	}
	...
	public function get_comp_days($last_days, $first_day,$first_date,$id,$emp_type,$work,$branch){ 
		...
		foreach($last_days as $comp_date){
			...
			if(($comp_date == $second_sat || $comp_date == $forth_sat)){ 
				$date_cond = array('and' => array('from_date <=' => $comp_date),  array('to_date >=' => $comp_date));
				$check_sat = $this->HrDeactivateSat->find('count', array('conditions' => array($date_cond,
				'app_users_id' => $id)));
				// if saturday not working
				if(empty($check_sat)){
					$comp_days[] = $comp_date;
				}	
			}
			...
			foreach($od_data as $data){
				if($comp_date == $data['HrLeave']['leave_from']){
					// disable if saturday is working
					if(empty($check_sat)){
						$comp_days[] = $comp_date;
					}
				}
	...
	public function eval_leave_status($id,$branch,$date,$doj){
		...
		return $year_present_split[0] + $year_present_split[1]+$year_holiday;
	}
	...
	public function calculate_late_deduct($id,$year){
		...
		as count"), 'conditions' => array('app_users_id' => $id, 'late_type' => 'S', 'HrLateHr.created_date like' => $year.'%')));
	...
	public function calculate_late_addition($id,$year){
		$this->loadModel('HrLateHr');
		$late_data = $this->HrLateHr->find('all', array('fields' => array("TIME_FORMAT(SEC_TO_TIME( SUM( TIME_TO_SEC(tot_hr))), '%k:%i') 
		as count"), 'conditions' => array('app_users_id' => $id, 'late_type' => 'A', 'HrLateHr.created_date like' => $year.'%')));
