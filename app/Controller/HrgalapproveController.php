...
class HrgalapproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Gallery - HRIS - BigOffice');		
	...
	public function process_gal($gal_id, $st_id, $status,$user_id){ 
		...
		if($this->HrGalApprove->HrGalStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your posted gallery is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_gallery', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
				// show the msg.								
	...
	public function view_gallery($id,$st_id){
		// set the page title		
		$this->set('title_for_layout', 'Gallery Request - Approve/Reject - HRIS - BigOffice');
