...
class HrleaveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Leave - HRIS - BigOffice');		
	...
	public function check_leave_days(){
		...
		while($diff >= 0){			
			...
			if($day == 6){ 
				...
				$check_sat = $this->check_sat_working($this->Session->read('USER.Login.id'),$from);
				// for even saturdays
				if($second_sat == $date[2] || $forth_sat == $date[2]){
					// if sat. working day
					if($check_sat){
						$count++;
					}
				}else{ // for odd saturdays
					$count++;	
				}
	...
	public function create_leave(){ 		
		// set the page title		
		$this->set('title_for_layout', 'Request Leave - HRIS - BigOffice');	
		//$gender = $this->set_user_gender();	
		// load leave types
		$this->load_leave_types($this->Session->read('USER.Login.gender'),$this->Session->read('USER.Login.id'),$this->Session->read('USER.Login.doj'),$this->Session->read('USER.Login.emp_type'));
		// get leaves remaining
							...
							if($this->HrLeaveStatus->save($data, true, $fieldList = array('hr_leave_id','created_date','app_users_id'))){						
								...
								foreach($approval_data as $hr_data){
									if($superior_data['Home']['id'] != $hr_data['Home']['id'] && $hr_data['Home']['id'] != $this->Session->read('USER.Login.id')){
										...
										if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' created leave request (No Action Required)!', 'leave_creation', 'noreply@mypdca.in', $hr_data['Home']['email_address'],$vars)){	
											// show the msg.								
	...
	public function check_no_days(){
		if(empty($this->request->data['HrLeave']['no_days']) || $this->request->data['HrLeave']['no_days'] < '0.5'){
	...
	public function view_leave($id){
		// set the page title		
		$this->set('title_for_layout', 'View Leave - HRIS - BigOffice');
		if(!empty($id) && intval($id)){
			// authorize user before action
			$ret_value = $this->auth_action($id);
			if($ret_value == 'pass'){
				$this->HrLeave->bindModel(array('hasOne' => array('HrLeaveComp', 'HrCancelLeaveStatus')));
				$data = $this->HrLeave->findById($id, array('fields' => 'leave_from', 'session','HrLeave.id','HrCancelLeaveStatus.id','leave_to','no_days','reason','HrLeaveType.desc','group_concat(distinct HrLeaveComp.comp_off order by HrLeaveComp.comp_off asc) as compoff'));
