...
class TskroaapproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve ROA - Work Planner - BigOffice');
	...
	public function process_req($req_id, $st_id, $status,$user_id){ 
		...
		if($this->TskRoaApprove->TskApplauseStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your ROA request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_applause', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){		
				// show the msg.								
		...
		if($this->TskRoaStar->save($data, true, $fieldList = array('star_type','created_date','created_by','tsk_applause_id'))){			
			...
			*/
			...
			foreach($user_data as $user){ 			
				...
				// notify employee						
				if(!$this->send_email('BigOffice - Congrats! You have been selected as - '.$st_msg, 'notify_applause_star', 'noreply@mypdca.in', $user['HrEmployee']['email_address'],$vars)){		
					// show the msg.								
					$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
				}else{
	...
	public function notify_employee($name, $email){
		...
		foreach($email_address as $key => $mail){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Congrats! You have been selected for Round of Applause this month', 'notify_applause_star', 'noreply@mypdca.in', $mail, $vars)){		
				// show the msg.								
				$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
			}else{
	...
	public function view($id,$st_id,$user_id){
		$this->layout = 'iframe';
		// set the page title		
		$this->set('title_for_layout', 'View ROA - Work Planner - BigOffice');
