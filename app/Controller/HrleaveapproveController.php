...
class HrleaveApproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Leave - HRIS - BigOffice');
		...
		// fetch the advances		
		$this->paginate = array('fields' => array('id','session','leave_from', 'leave_to','reason', 'no_days','HrLeaveType.desc', 'created_date', 
	...
	public function process_adv($leave_id, $st_id, $status,$user_id){ 
		...
		if($this->HrLeaveApprove->HrLeaveStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your leave request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_leave', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
				// show the msg.								
			...
			if($status == 'A'){					
				...
				if(!empty($superior_data)){
					// notify employee						
					if(!$this->send_email('BigOffice - Leave request is rejected by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_leave', 'noreply@mypdca.in', $superior_data['Home']['email_address'],$vars)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
					}
		...
		// iterate finance team
			foreach($hr_data as $fin){	
				...
				if($this->Session->read('USER.Login.id') != $fin['Home']['id']){
					...
					$sub = 'BigOffice - Leave request is '.$status.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'));
	...
	public function view_leave($id,$st_id){
		// set the page title		
		$this->set('title_for_layout', 'Leave Request - Approve/Reject - HRIS - BigOffice');
		if(!empty($id) && intval($id) && !empty($st_id) && intval($st_id)){
			...
			if($ret_value == 'pass'  || $ret_value == 'view_only'){	
				...
				'created_date','Home.first_name','HrLeaveApprove.session','Home.last_name','HrLeaveStatus.remarks','group_concat(DISTINCT  HrLeaveComp.comp_off  order by HrLeaveComp.comp_off asc) as compoff'), 
