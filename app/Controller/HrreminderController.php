...
class HrreminderController extends AppController {  
	...
	public function att_change_reminder($from){ 
		...
		// fetch all users
		$user_data = $this->Home->find('all', array('conditions' => array('Home.status' => '1', 'Home.is_deleted' => 'N', 'Home.att_type !=' => 'N'), 
		...
		foreach($user_data as $user){  
			...
			if(count($att_status) > 0){
				...
				if(!$this->send_email('BigOffice - Attendance Reminder  - '.date('d-M-Y'), 'attendance_reminder', 'noreply@mypdca.in', $user['Home']['email_address'],$vars)){		
					// show the msg.								
