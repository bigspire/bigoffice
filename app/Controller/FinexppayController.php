...
class FinExpPayController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Pay Expense - Finance - BigOffice');
		// get employee list
		$this->loadModel('Home');
	...
	public function notify_employee($user_data,$pay_data,$exp_data){
		...
		if(!$this->send_email('BigOffice - Expense submission payment updated by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_exp_pay', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
			// show the msg.								
	...
	public function view_payment($id, $pay_id){
		// set the page title		
		$this->set('title_for_layout', 'View Payment Advance - Finance - BigOffice');		
