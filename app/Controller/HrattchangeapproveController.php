...
class HrattchangeapproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Attendance Change - HRIS - BigOffice');
	...
	public function process_adv($att_id, $st_id, $status,$user_id){ 
		...
		if($this->HrAttChangeApprove->HrAttStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your attendance change request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_att_change', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
				// show the msg.								
			...
			if($status == 'R'){			
				...
				if(!empty($superior_data)){
					// notify employee						
					if(!$this->send_email('BigOffice - Attendance change request is rejected by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_att_change', 'noreply@mypdca.in', $superior_data['Home']['email_address'],$vars)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
						}
		...
		// iterate finance team
			foreach($hr_data as $hr){	
				...
				if($this->Session->read('USER.Login.id') != $hr['Home']['id']){
					...
					$sub = 'BigOffice - Attendance change request is '.$status.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'));
	...
	public function view_change($id,$st_id){
		// set the page title		
		$this->set('title_for_layout', 'Attendance Change - Approve/Reject - HRIS - BigOffice');
