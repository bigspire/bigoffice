...
class FinAdvanceController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Advance - Finance - BigOffice');
	...
	public function create_advance(){ 
		// set the page title		
		$this->set('title_for_layout', 'Create Advance - Finance - BigOffice');
		$this->load_customers();
		if ($this->request->is('post')){ 
			// validates the form
						...
						if($this->FinAdvStatus->save($data, true, $fieldList = array('fin_advance_id','created_date','app_users_id'))){						
							...
							foreach($approval_data as $fin_data){
								if($superior_data['Home']['id'] != $fin_data['Home']['id']){
									...
									// notify superiors						
									if(!$this->send_email('BigOffice - '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')).' created advance request (No Action Required)!', 'advance_creation', 'noreply@mypdca.in', $fin_data['Home']['email_address'],$vars)){	
										// show the msg.								
	...
	public function view_advance($id){
		// set the page title		
		$this->set('title_for_layout', 'View Advance - Finance - BigOffice');
