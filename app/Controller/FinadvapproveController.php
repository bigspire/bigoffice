...
class FinAdvApproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Advance - Finance - BigOffice');
	...
	public function process_adv($adv_id, $st_id, $status,$user_id){ 
		...
		if($this->FinAdvApprove->FinAdvStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your advance request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_advance', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
				// show the msg.								
			...
			if($status == 'A'){					
				...
				if(!empty($superior_data)){				
					...
					// notify employee						
					if(!$this->send_email('BigOffice - Advance request is rejected by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_advance', 'noreply@mypdca.in', $superior_data['Home']['email_address'],$vars)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
					}
		...
		// iterate finance team
			foreach($fin_data as $fin){	
				...
				// change the subject
					...
					$sub = 'BigOffice - Advance request is '.$status.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'));
	...
	public function view_advance($id,$st_id){
		// set the page title		
		$this->set('title_for_layout', 'Advance Request - Approve/Reject - Finance - BigOffice');
