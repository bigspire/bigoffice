...
class TvlreqaprController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Travel - Biz Tour - BigOffice');
	...
	public function process_req($tvl_id, $st_id, $status,$user_id){ 
		...
		if($this->TvlReqApr->TvlReqStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your travel request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_travel', 'noreply@mypdca.in', $user_data['HrEmployee']['email_address'],$vars)){		
				// show the msg.								
			...
			if($status == 'A'){				
				...
				if(!empty($superior_data)){				
					...
					// notify employee						
					if(!$this->send_email('BigOffice - Travel request is rejected by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_travel', 'noreply@mypdca.in', $superior_data['HrEmployee']['email_address'],$vars)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
					}
		...
		// iterate travel admin team
			foreach($tvl_data as $tvl){	
				...
				// change the subject
					...
					$sub = 'BigOffice - Travel request is '.$status.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'));
