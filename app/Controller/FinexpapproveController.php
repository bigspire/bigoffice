...
class FinExpApproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Expense - Finance - BigOffice');
		// get employee list
	...
	function notify_employee($exp_id, $user_id){	
		...
		if(!$this->send_email('BigOffice - Your expense request is processed by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_expense', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
			// show the msg.								
	...
	public function view_expense($id,$user_id, $st_id){
		// set the page title		
		$this->set('title_for_layout', 'Expense Request - Approve/Reject - Finance - BigOffice');
