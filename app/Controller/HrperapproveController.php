...
class HrperapproveController extends AppController {  
	...
	public function index(){	
		// set the page title
		$this->set('title_for_layout', 'Approve Permission - HRIS - BigOffice');
	...
	public function process_adv($perm_id, $st_id, $status,$user_id){ 
		...
		if($this->HrPerApprove->HrPerStatus->save($data, true, $fieldList = array('modified_by','modified_date','remarks','status'))){
			...
			// notify employee						
			if(!$this->send_email('BigOffice - Your permission request is '.$st_msg.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_permission', 'noreply@mypdca.in', $user_data['Home']['email_address'],$vars)){		
				// show the msg.								
			...
			if($status == 'R'){			
				...
				if(!empty($superior_data)){
					// notify employee						
					if(!$this->send_email('BigOffice - Permission request is rejected by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')), 'notify_permission', 'noreply@mypdca.in', $superior_data['Home']['email_address'],$vars)){		
						// show the msg.								
						$this->Session->setFlash('<button type="button" class="close" data-dismiss="alert">&times;</button>Problem in sending the mail to user...', 'default', array('class' => 'alert alert-error'));				
					}else{								
						}
		...
		// iterate finance team
			foreach($fin_data as $fin){	
				...
				if($this->Session->read('USER.Login.id') != $fin['Home']['id']){
					...
					$sub = 'BigOffice - Permission request is '.$status.' by '.ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name'));
	...
	public function view_permission($id,$st_id){
		// set the page title		
		$this->set('title_for_layout', 'Permission Request - Approve/Reject - HRIS - BigOffice');
