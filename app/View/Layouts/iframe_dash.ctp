...
<head>
	...
	/* function to load php value into form */
	if($('.sheepitWaive').length > 0){ 
		for(i = 0; i <= $('#tot_edit').val(); i++){
			if($('#Wreason'+i).length > 0){ 
				$('#reason'+i).val($('#Wreason'+i).val());
			}
			if($('#Wdate'+i).length > 0){
				$('#date'+i).val($('#Wdate'+i).val());
			}			
		}
	}
		...
		/* validate the request waive off form */
		$('.waiveSend').click(function() {
			$('#is_draft').val('');
			$('.draftMsg').hide();
			res = validateFormWaive();			
			return res;
		});
	...
	function validateFormWaive () { 
		...
		if (submit == false) {
			...
			$('.errorMsg').hide();
			// add confirmation
			if(confirm('Are you sure you want to send? only one request can be sent in a month!')){
				return true;
			}else{
				return false;
			}
