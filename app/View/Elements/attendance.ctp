...
<div class="btn-group" style="position:absolute;right:35px;top:43px;">
...
<span class="icon-ok icon-on-right"></span> Approve	Attendance
<?php if($appr_count > 0):?>
<span class="badge badge-warning stCount" style="top:-10px"><?php echo $appr_count; ?></span>
<?php endif; ?></button></a>
<?php endif; ?>
<?php if(!empty($apprv_att)) :?><br><br><br>
							<div class="btn-group">
...
<br><br>		<br>	
	<?php endif; ?>
			...
			<!--a style="float:left;" href="<?php echo $this->webroot;?>home/hr_spoc/" rel="tooltip"  title="SPOC for HR Processes" class="iframeBox click_hide" val="75_85"><button class="btn-success btn  btn-xs dropdown-toggle" style="margin-right:5px">
<span class="icon-info-sign icon-on-right"></span> HR SPOC
</button>
</a-->		
	...
	<?php if($waive_done):?>
<a style="float:left;" href="<?php echo $this->webroot;?>home/view_waiveoff/" rel="tooltip"  title="Waive-Off Requested" class="iframeBox click_hide" val="50_65"><button class="btn-green btn  btn-xs dropdown-toggle" style="margin-right:5px">
<span class="icon-edit icon-on-right"></span>  Waive-Off Requested	
</button>
</a>	
<?php else: ?>
<a style="float:left;" href="<?php echo $this->webroot;?>home/request_waiveoff/" rel="tooltip"  title="Request Late Mins. Waive-Off" class="iframeBox click_hide" val="50_80"><button class="btn-yellow btn  btn-xs dropdown-toggle" style="margin-right:5px">
<span class="icon-edit icon-on-right"></span> Request Waive-Off	
</button>
</a>
<?php endif; ?>						
...
<button  style="margin-left:50px" data-toggle="dropdown" class="btn btn-minier btn-white dropdown-toggle click_hide" rel="tooltip" data-placement="right" style="border-width:0px"  title="Rewind (Past Attendance)">
			...
			for($j = 1; $j <= $no_days; $j++):
				if(!empty($in_data[$j])):
					...
					$leave_rec = $this->Functions->check_leave_taken($day_no,$month[1], $month[0],$in_data[$j], $leave_data_user, $holidayList, $happyLeave,$satWork);					
					if(empty($leave_rec)): 
...
<table width="590" style="margin-top:35px;float:left;width:590px" id="mytable" class="table table-bordered table-striped table-hover">
						...
						<td width="120">
								...
								<?php $leave_rec = $this->Functions->check_leave_taken($day_no,$month[1], $month[0],$in_data[$i], $leave_data_user, $holidayList, $happyLeave,$satWork);
								if(empty($leave_rec)):?>
