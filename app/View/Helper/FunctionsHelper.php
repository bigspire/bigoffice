...
class FunctionsHelper extends AppHelper {
	...
	public function get_att_punch_type($type){
		return ($type == 'B' ? 'Biometric' : ($type == 'O' ? 'Online' : 'Not Applicable'));	
	}
	...
	public function check_leave_taken($date, $month, $year, $in_time, $leaves, $holidays, $happy,$sat_work){	
   ...
   public function get_admin_status($status,$st_created, $st_user){
	if(!empty($status)){
		...
		$status_show = $status == 'W' || $status == 'P' ? 'Pending': ($status == 'R' ? 'Rejected' : 'Approved');
		$st_label .= "<span class='label $st_color'><a href='#' rel='tooltip' data-original-title = '".$details."'>".$status_show.'</a></span>';	
		return $st_label;
		}	
   }
   
     /* show the status of the waive off req */
   public function get_waive_status($status){
	if(!empty($status)){
		$st_color = ($status == 'A' ? 'green' : ($status == 'R' ? 'red' : ''));
		$status_show = $status == 'P' ? 'Pending': ($status == 'R' ? 'Rejected' : 'Approved');
		$st_label .= "<span class='$st_color'>".$status_show.'</span>';	
		return $st_label;
		}	
   }
