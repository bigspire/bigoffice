...
class HrLeave extends AppModel {
	...
	public $validate = array(	  
		...
		'hr_leave_type_id' => array(		
			...
			'check_eligible' => array(
			 ...
			 'check_pl_limit' => array(
                'rule'     => 'check_pl_limit',
                'required' => true,
                'message'  => 'You can take max. of 10 PL in a month'
            )
	...
	/* function to validate the max. PL */
	public function check_pl_limit(){
		// check no. of days more than 10 days
		if($this->data['HrLeave']['no_days'] > 10){
			return false;
		}else{
			$this->unBindModel(array('hasOne' => array('HrLeaveStatus')));
			$date_str = $this->get_search_date($this->data['HrLeave']['leave_from']);
			$data = $this->find('all', array('fields' => array('sum(HrLeave.no_days) as total'), 'conditions' => array('HrLeave.app_users_id' => CakeSession::read('USER.Login.id'), 'HrLeave.leave_from like' => $date_str.'%',
			'HrLeave.is_approve !=' => 'R', 'HrLeave.is_deleted'=> 'N', 'HrLeave.hr_leave_type_id' => 2)));	
			if($data[0][0]['total'] > 10 && $this->data['HrLeave']['hr_leave_type_id'] == '2'){
				return false;
			}else{
				return true;
			}
	...
	public function get_applied_leaves(){
		...
		if(CakeSession::read('USER.Login.emp_type') == 'A' || CakeSession::read('USER.Login.probation') == 'Y'){
